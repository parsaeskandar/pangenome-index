...
// and its reverse complement. So this function returns the sorted vector of kmers
    ...
    // Complete version that extracts both forward and reverse strand kmers
    template<class KeyType>
    std::vector <Kmer<KeyType>>
...
// This function returns the unique kmers in the graph and stores them in the index (forward strand only)
    template<class KeyType>
    void
...
//    gbwtgraph::for_each_nonredundant_window(graph, k, hash_kmers, true);
    ...
    // Complete version that processes both forward and reverse strand kmers
    template<class KeyType>
    void
    unique_kmers_parallel_complete(const GBWTGraph &graph,
                                   hash_map <gbwtgraph::Key64::value_type, gbwtgraph::Position> &index,
                                   size_t k) {
        typedef KeyType key_type;
        typedef Kmer <key_type> kmer_type;
...
//        cerr << kmers.size() << endl;
            ...
            for (auto kmer: kmers) {
                ...
                // For reverse strand kmers, we need to adjust the position
                pos_t pos;
                if (kmer.is_reverse) {
                    // For reverse strand kmers, we need to reverse the position
                    pos = pos_t{graph.get_id(*iter), !graph.get_is_reverse(*iter), 
                               graph.get_length(*iter) - 1 - (kmer.offset - node_start)};
                } else {
                    // For forward strand kmers, use the original position
                    pos = pos_t{graph.get_id(*iter), graph.get_is_reverse(*iter), kmer.offset - node_start};
                }
