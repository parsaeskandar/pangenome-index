...
template<typename T>
bpNode<T>::bpNode(std::size_t degree, bool leaf) : leaf(leaf), degree(degree) {
...
// set next
template<typename T>
void bpNode<T>::set_next(bpNode <T> *next_node) {
    next = next_node;
}

// set prev
template<typename T>
void bpNode<T>::set_prev(bpNode <T> *prev_node) {
    prev = prev_node;
}

// set parent
template<typename T>
void bpNode<T>::set_parent(bpNode <T> *parent_node) {
    parent = parent_node;
}

// add child
template<typename T>
void bpNode<T>::add_child(bpNode <T> *child) {
    children.push_back(child);
}

// add item
template<typename T>
void bpNode<T>::add_item(T item) {
    items.push_back(item);
}

// add item in ith position
template<typename T>
void bpNode<T>::add_item(T item, size_t i) {
    items.insert(items.begin() + i, item);
}

// add child in ith position
template<typename T>
void bpNode<T>::add_child(bpNode <T> *child, size_t i) {
    children.insert(children.begin() + i, child);
}

// change ith item to new item
template<typename T>
void bpNode<T>::change_item(T new_item, size_t i) {
    items[i] = new_item;
...
// get parent node
template<typename T>
bpNode <T> *bpNode<T>::get_parent() {
    return parent;
}

// find the index of this node in the parent
template<typename T>
int bpNode<T>::find_index_in_parent() {
...
template<typename T>
bool bpNode<T>::is_underflowing() const {
    return items.size() < degree / 2;
}

// remove the item at ith position
template<typename T>
void bpNode<T>::remove_item(size_t i) {
    items.erase(items.begin() + i);
}

// remove the child at ith position
template<typename T>
void bpNode<T>::remove_child(size_t i) {
...
template<typename T>
bool bpNode<T>::is_full() const {
    return items.size() == items.capacity();
}

template<typename T>
bool bpNode<T>::is_leaf() const {
    return leaf;
}

template<typename T>
bool bpNode<T>::is_root() const {
    return parent == nullptr;
}

template<typename T>
bool bpNode<T>::is_empty() const {
    return items.empty();
}

// get items
template<typename T>
std::vector <T> bpNode<T>::get_items() {
    return items;
}
...
bpNode<T>::get_children() {
    return children;
}

// get next node
template<typename T>
bpNode <T> *bpNode<T>::get_next() {
    return next;
}

// get previous node
template<typename T>
bpNode <T> *bpNode<T>::get_prev() {
    return prev;
}

template<typename T>
void bpNode<T>::remove_first_item() {
...
template<typename T>
void bpNode<T>::remove_last_item() {
    items.pop_back();
}
...
template<typename T>
void bpNode<T>::remove_last_child() {
    children.pop_back();
}

template<typename T>
int bpNode<T>::get_size() {
    return items.size();
}

// get ith children
template<typename T>
bpNode <T> *bpNode<T>::get_child(size_t i) {
    bpNode <T> *child = children[i];
    return child;
}

template<typename T>
T bpNode<T>::get_item(size_t i) {
    return items[i];
}
...
template<typename T>
void bpNode<T>::replace_items_leaf(vector <T> new_items) {
...
// replace children
template<typename T>
void bpNode<T>::replace_children(vector<bpNode < T> *
...
*/
template<typename T>
vector <T> bpNode<T>::insert(const T &data, size_t run_length) {
...
template<typename T>
vector <T> bpNode<T>::insert_success(const T &data, size_t run_length, bool &success) {
//        if (DEEBUG) cout << "inserting into node " << data << " " << run_length << endl;
    success = true;
    assert(leaf);
    size_t inserted_index;
...
template<typename T>
int bpNode<T>::search_non_leaf(const T &data) {
 ...
 */
template<typename T>
vector <T> bpNode<T>::run_insert(vector <T> new_items, T new_run, int run_length, size_t &inserted_index) {
...
template<typename T>
bool bpNode<T>::merge_item_next(vector <T> &new_items) {
...
template<typename T>
bool bpNode<T>::merge_item_prev(vector <T> &new_items) {
    assert(prev != nullptr);
...
template<typename T>
void BplusTree<T>::parent_insert(bpNode <T> *node, T data, bpNode <T> *child) {
...
template<typename T>
void BplusTree<T>::remove_from_parent(bpNode <T> *node) {
...
template<typename T>
void BplusTree<T>::handle_parent_underflow(bpNode <T> *node) {
...
template<typename T>
void BplusTree<T>::leaf_underflow(bpNode <T> *node) {
...
template<typename T>
BplusTree<T>::BplusTree(std::size_t _degree) : degree(_degree), root(nullptr) {}
...
// get root
template<typename T>
bpNode <T> *BplusTree<T>::get_root() {
    return root;
}

template<typename T>
bpNode <T> *BplusTree<T>::leaf_search(bpNode <T> *node, T data) {
    assert(node != nullptr);
...
template<typename T>
size_t *BplusTree<T>::search_index(T data) {
...
template<typename T>
void BplusTree<T>::insert(const T &data, size_t run_length) {
    insert_success(data, run_length);
}

template<typename T>
bool BplusTree<T>::insert_success(const T &data, size_t run_length) {
    bool success = true;
    if (root == nullptr) {
...
template<typename T>
void BplusTree<T>::print(bpNode <T> *node) {
...
template<typename T>
void BplusTree<T>::print_whole() {
    print(root);
}

template<typename T>
size_t BplusTree<T>::get_bpt_size() {
    return get_size(this->root);
}

template<typename T>
size_t BplusTree<T>::get_size(bpNode <T> *cursor) {
    size_t size = 0;
    if (cursor != NULL) {
        if (cursor->is_leaf()) {
...
template<typename T>
size_t BplusTree<T>::get_bpt_run_count() {
    return get_run_count(this->root);
}

template<typename T>
size_t BplusTree<T>::get_run_count(bpNode <T> *cursor) {
    size_t count = 0;
    if (cursor != NULL) {
        if (cursor->is_leaf()) {
            for (int i = 0; i < cursor->get_size(); ++i) {
...
template<typename T>
BplusTree<T>::Iterator::Iterator(bpNode <T> *node, int index) : node(node), index(index) {}

// Dereference operator: returns the item at the current index
template<typename T>
T BplusTree<T>::Iterator::operator*() const {
    return node->get_item(index);
}

// Pre-increment operator: Move to the next item in the tree
template<typename T>
...
// Pre-decrement operator: Move to the previous item in the tree
template<typename T>
typename BplusTree<T>::Iterator &BplusTree<T>::Iterator::operator--() {
    if (node != nullptr) {
        index--;
...
// Not-equal operator: Check if two iterators are different
template<typename T>
bool BplusTree<T>::Iterator::operator!=(const Iterator &other) const {
    return node != other.node || index != other.index;
}

// Equal operator: Check if two iterators are the same
template<typename T>
bool BplusTree<T>::Iterator::operator==(const Iterator &other) const {
    return node == other.node && index == other.index;
}

// Begin method: Returns an iterator to the first item in the tree
template<typename T>
typename BplusTree<T>::Iterator BplusTree<T>::begin() {
    bpNode <T> *current = root;
    if (!current) return Iterator(nullptr, 0);
    while (current && !current->is_leaf()) {
        current = current->get_child(0);  // Navigate to the leftmost leaf node
    }
    return Iterator(current, 0);
}

// End method: Returns an iterator representing "past-the-end" (nullptr)
template<typename T>
